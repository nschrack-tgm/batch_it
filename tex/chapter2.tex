...
Dies ist die Sammelstelle für Echtzeitdaten, die von RTUs generiert werden. Meist sind diese Stationen herkömmliche Computer.\\ 
\\
...
Eine Ontologie ist ein auf \ac{XML}-basierendes Konstrukt, das einem das Abbilden von Dingen und deren Relationen ermöglicht. Um solch komplexe Zustände darzustellen, stehen neben einfachen Entitäten (Dinge) auch Eigenschaften zur Verfügung.\\\\
Im philosophischen Sinne von der Stanford Encyclopedia of Philosophy:\\
...
Dies ermöglicht es Modelle aufzubauen die komplexe Netzwerke darstellen.\\
\\
...
\glqq An Activity is a kind of Behavior that is specified as a graph of nodes interconnected by edges. A subset of the nodes are executable nodes that embody lower-level steps in the overall Activity. Object nodes hold data that is input to and output from executable nodes, and moves across object flow edges.\grqq \cite{UML_def}\\
\\ 
Ein Startknoten (Kontrollknoten) ist der Startpunkt für eine Sequenzielle Abfolge von Aktionen. Die Reihenfolge wird durch Kanten (Kontrollfluss) festgelegt. Am Ende solch einer Abfolge steht ein Endknoten (ebenfalls ein Kontrollknoten) oder ein Objekt welches das Endprodukt einer Abfolge ist. Zwischen Start- und Endknoten liegen ausführbare Aktionen die der essentielle Teil des Aktivitätsdiagramms sind. Zwischen zwei Aktionen können Objekte, Zwischenprodukte, etc. weitergeleitet werden, wodurch der Kontrollfluss zu einem Objektfluss wird. Solche Objekte werden entweder als nicht abgerundete Aktionen dargestellt oder aber als ein Pin (kleines Kästchen) an den Aktionen selbst.\\
\\
\glqq Activities may describe procedural computation, forming hierarchies of Activities invoking other Activities, or, in an object- oriented model, they may be invoked indirectly as methods bound to Operations that are directly invoked.\grqq \cite{UML_def}\\
\\

\begin{figure}[hbt!]
 \centering
  \includegraphics[width=0.7\textwidth]{graphics/stateoftheart/Activity_bsp}

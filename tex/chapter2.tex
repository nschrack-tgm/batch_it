...
\chapter{State of the Art} \label{chapter:stateoftheart}
 ...
 Im Detail gehört zum Aufbau einer \ac{SPS} eine Stromversorgung, eine Verarbeitungseinheit (\ac{CPU}), digitale sowie analoge I/O Anschlüsse, eine Feldbusschnittstelle und das Programmiergerät, wobei es sich dabei heutzutage eigentlich ausschließlich um einen externen Computer handelt. Die Stromversorgung \ac{PS} wandelt gleichzeitig die Netzspannung in eine 24-V-Gleichspannung um, mit der die Elektronik der \ac{SPS} versorgt wird.\\
	...
	\lstinputlisting[caption=Beispiel einer Anweisungsliste,style=ST]{extra/awl.txt}
	...
	Eine Sonderstellung unter den Sprachen zur Programmierung einer \ac{SPS} nimmt die Ablaufsprache ein. Da die Inhalte dieser entweder grafisch oder textuell definiert werden, wird ein recht breit gefächertes Feld an Anwendern abgedeckt.\\
...
Der zenon Supervisor enthält schon bei der Installation Wizards zum Importieren sowie auch Exportieren von in \ac{XML} gespeicherten Daten und zum Erzeugen von Variablen.\\
